<div>
            ...
            <div class="w-full relative inline-flex">
                <select required id="warehouse_id" name="warehouse_id" wire:model="warehouse_id"
                    ...
                    @if (settings()->default_warehouse_id == true)
                        <option value="{{ $default_warehouse?->id }}" selected>{{ $default_warehouse?->name }} (default)</option>
                    @endif
    ...
    <div class="mb-4 d-flex justify-center flex-wrap">
        <x-button danger type="button" wire:click="calculateTotal" wire:loading.attr="disabled" class="ml-2 font-bold">
            {{ __('Calculate') }}
        </x-button>
        <button
            ...
            <form id="checkout-form" wire:submit.prevent="store" class="py-5">
                ...
                <div class="float-left pb-4 px-2">
                    <x-button secondary type="button"
                        wire:click="closeModal">{{ __('Close') }}</x-button>
