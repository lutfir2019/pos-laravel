...
class Transactions extends Component
{
    ...
    {
        $this->startDate = now()->startOfYear()->format('Y-m-d') . ' 00:00:00';
        $this->endDate = now()->endOfDay()->format('Y-m-d') . ' 23:59:59';
        ...
        $this->stockValue = ProductWarehouse::whereBetween('created_at', [$this->startDate, $this->endDate])->sum(DB::raw('qty * cost'));

        $this->lastSales = Sale::with(['customer', 'saleDetails.product.warehouses'])
            ->latest()
            ->take(5)
